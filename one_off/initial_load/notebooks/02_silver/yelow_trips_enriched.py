...
#df_trips.display()
#so now we add the zone or borough id here
...
# we can join on pu_locationid and a second join
#on do_location_id and join again on locaiton id
df_join_1 = df_trips.join(df_zones, df_trips.pu_location_id == df_zones.location_id, "left"
).select(
    df_trips.vendor,
    ...
    df_trips.du_location_id, # still need to read in the location id
...
df_join_final = df_join_1.join(df_zones, df_join_1.du_location_id == df_zones.location_id, "left"
).select(
    df_join_1.vendor,
...
#we couldve done the join together but breaking it down to make it easier
